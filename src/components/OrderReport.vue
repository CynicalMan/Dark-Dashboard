<template>
    ...
    <table class="w-full">
        ...
        <tbody>
            <tr v-for="order in orders" class="text-sm text-gray-500">
                <td class="py-4 ">
                    <div class="flex gap-4 items-center">
                        <img width="32" :src="`./src/assets/avatars/${order.avatar}`" alt="gg">
                ...
                <td class="py-4 flex justify-center">
                    <span class="flex justify-center py-1 w-24 font-medium capitalize rounded-full" 
                    :class="{
                        'bg-accent-green/20 text-accent-green' : order.status === 'completed',
                        'bg-accent-purple/20 text-accent-purple' : order.status === 'preparing',
                        'bg-accent-orange/20 text-accent-orange' : order.status === 'pending',
                    }">
                        {{ order.status }}
                    </span>
                </td>
